...
export default function RoasCalculatorPage() {
    ...
    const [results, setResults] = useState<{
        standardRoas: number; realRoas: number; netProfit: number
        profitMargin: number; costPerOrder: number; returnLoss: number
        effectiveRevenue: number
    } | null>(null)

    // Check user on mount + listen for auth changes (Google popup return)
    ...
    useEffect(() => {
        ...
        // Listen for visibility changes (user returns from Google auth tab)
    ...
    // Google: open in new tab, stay on this page
    const handleGoogleAuth = async () => {
        setAuthLoading(true)
        setAuthMode('google-waiting')
    ...
    const handleOtpVerify = async () => {
        ...
        setAuthLoading(false)
        if (error) {
            alert('Kod geçersiz veya süresi dolmuş. Tekrar deneyin.')
        } else if (data.user) {
            setUser(data.user)
            setAuthMode('idle')
        }
    ...
    return (
        <div className="py-12 px-6">
            <div className="max-w-2xl mx-auto">
                {/* Header */}
                <div className="mb-10">
                    <p className="text-xs font-medium tracking-[0.2em] uppercase text-violet-600 mb-2">Ücretsiz Araç</p>
                    <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900 mb-3">ROAS Simülatörü</h1>
                    <p className="text-sm text-gray-500">Reklam harcamalarınızın gerçek geri dönüşünü hesaplayın. İade, kargo ve komisyon maliyetleri dahil.</p>
                </div>
                ...
                <div className="rounded-2xl border border-gray-200/80 bg-white p-6 sm:p-8 mb-8">
                    ...
                    <button onClick={handleCalculate} className="mt-6 w-full py-3 rounded-xl bg-violet-600 text-white text-sm font-semibold hover:bg-violet-700 transition-colors">Hesapla</button>
                </div>
                ...
                {calculated && results && (<>
                    ...
                    <div className="rounded-2xl border border-gray-200/80 bg-white p-6 sm:p-8 mb-6">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <ResultCard label="Standart ROAS" value={`${results.standardRoas}x`} color={results.standardRoas >= 3 ? 'green' : results.standardRoas >= 2 ? 'yellow' : 'red'} />
                            <ResultCard label="Gerçek ROAS" value={`${results.realRoas}x`} color={results.realRoas >= 2.5 ? 'green' : results.realRoas >= 1.5 ? 'yellow' : 'red'} highlight />
                        </div>
                    ...
                    <div className="relative rounded-2xl border border-gray-200/80 bg-white p-6 sm:p-8 overflow-hidden">
                        ...
                        <div className={!user ? 'blur-md select-none pointer-events-none' : ''}>
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between items-center py-2 border-b border-gray-100"><span className="text-sm text-gray-500">Brüt Gelir</span><span className="text-sm font-semibold text-gray-900">₺{parseFloat(revenue).toLocaleString('tr-TR')}</span></div>
                                <div className="flex justify-between items-center py-2 border-b border-gray-100"><span className="text-sm text-gray-500">İade Sonrası Efektif Gelir</span><span className="text-sm font-semibold text-gray-900">₺{results.effectiveRevenue.toLocaleString('tr-TR')}</span></div>
                                <div className="flex justify-between items-center py-2 border-b border-gray-100"><span className="text-sm text-gray-500">Toplam Maliyet</span><span className="text-sm font-semibold text-red-600">₺{results.costPerOrder.toLocaleString('tr-TR')}</span></div>
                            ...
                            <div className="mt-6 p-4 rounded-xl bg-violet-50 border border-violet-200/50">
                                ...
                                <p className="text-xs text-violet-600 leading-relaxed">
                                    {results.profitMargin < 5 ? 'Kâr marjınız kritik seviyede düşük. Ürün maliyetlerinizi veya fiyatlandırmanızı gözden geçirmenizi öneriyoruz.'
                                        : results.profitMargin < 15 ? 'Kâr marjınız kabul edilebilir ancak iyileştirme alanı var. İade oranını düşürmek en hızlı kazanım olabilir.'
                        ...
                        {!user && (
                            <div className="absolute inset-0 flex items-center justify-center bg-white/60 backdrop-blur-sm rounded-2xl">
                                <div className="text-center max-w-sm px-6">
                                    ...
                                    <h3 className="text-lg font-bold text-gray-900 mb-2">Bu ROAS ile Ay Sonunda Cebinize Ne Kalır?</h3>
                                    <p className="text-sm text-gray-500 mb-6">Detaylı kârlılık analizini görmek için ücretsiz giriş yapın.</p>

                                    {/* Google Auth — opens new tab */}
                                    {authMode !== 'otp-verify' && (
                                    ...
                                    {/* Google waiting state */}
                                    {authMode === 'google-waiting' && (
                                        <p className="text-xs text-violet-600 mb-3 animate-pulse">Yeni sekmede giriş yapın, bu sayfa otomatik güncellenecek...</p>
                                    )}

                                    {/* Email: step 1 — enter email */}
                                    {authMode === 'idle' && (
                                        <button onClick={() => setAuthMode('email-input')} className="w-full py-3 rounded-xl bg-violet-600 text-white text-sm font-semibold hover:bg-violet-700 transition-colors">E-posta ile Giriş Yap</button>
                                    )}

                                    {authMode === 'email-input' && (
                                        <div className="flex gap-2">
                                            <input type="email" placeholder="ornek@email.com" value={emailInput} onChange={e => setEmailInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleEmailSubmit()} className="flex-1 px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30" />
                                            <button onClick={handleEmailSubmit} disabled={authLoading || !emailInput} className="px-4 py-3 rounded-xl bg-violet-600 text-white text-sm font-semibold disabled:opacity-50">Gönder</button>
                                        </div>
                                    )}
...
function InputField({ label, value, onChange, placeholder }: {
    ...
    return (
        <div>
            ...
            <input type="number" value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 transition-all" />
        </div>
    )
}
