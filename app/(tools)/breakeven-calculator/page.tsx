            ...
            {calculated && results && (<>
                <div className="rounded-2xl border border-gray-200/80 bg-white p-6 sm:p-8 mb-6">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        <div className="p-5 rounded-xl bg-blue-50 border border-blue-200/60 text-center">
                            <p className="text-xs font-medium text-blue-600/70 mb-1">Başa Baş Noktası</p>
                            <p className="text-3xl font-bold text-blue-700">{results.breakevenUnits}</p>
                            <p className="text-xs text-blue-500 mt-1">adet / ay</p>
                        </div>
                        <div className="p-5 rounded-xl bg-gray-50 border border-gray-200/60 text-center">
                            <p className="text-xs font-medium text-gray-500 mb-1">Gerekli Ciro</p>
                            <p className="text-3xl font-bold text-gray-800">₺{results.breakevenRevenue.toLocaleString('tr-TR')}</p>
                ...
                <div className="relative rounded-2xl border border-gray-200/80 bg-white p-6 sm:p-8 overflow-hidden">
                    ...
                    {!user && <SoftGate title="Farklı Senaryolarda Ne Kazanırsınız?" desc="Kâr senaryolarını görmek için ücretsiz giriş yapın." color="blue" onGoogle={authGoogle} onEmail={authEmail} loading={authLoading} emailInput={emailInput} setEmailInput={setEmailInput} showEmailForm={showEmailForm} setShowEmailForm={setShowEmailForm} />}
                </div>
            </>)}
        </div></div>
    )
...
function SoftGate({ title, desc, color, onGoogle, onEmail, loading, emailInput, setEmailInput, showEmailForm, setShowEmailForm }: { title: string; desc: string; color: string; onGoogle: () => void; onEmail: (e: string) => void; loading: boolean; emailInput: string; setEmailInput: (v: string) => void; showEmailForm: boolean; setShowEmailForm: (v: boolean) => void }) {
    return (<div className="absolute inset-0 flex items-center justify-center bg-white/60 backdrop-blur-sm rounded-2xl"><div className="text-center max-w-sm px-6">
        ...
        <button onClick={onGoogle} disabled={loading} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border border-gray-200 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors mb-3 shadow-sm">
        ...
        {!showEmailForm ? <button onClick={() => setShowEmailForm(true)} className={`w-full py-3 rounded-xl bg-${color}-600 text-white text-sm font-semibold hover:bg-${color}-700 transition-colors`}>E-posta ile Giriş Yap</button> : <div className="flex gap-2"><input type="email" placeholder="ornek@email.com" value={emailInput} onChange={e => setEmailInput(e.target.value)} className="flex-1 px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30" /><button onClick={() => onEmail(emailInput)} disabled={loading || !emailInput} className={`px-4 py-3 rounded-xl bg-${color}-600 text-white text-sm font-semibold disabled:opacity-50`}>Gönder</button></div>}
