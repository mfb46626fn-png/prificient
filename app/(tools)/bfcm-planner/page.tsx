...
export default function BfcmPlannerPage() {
    ...
    const handleEmailSubmit = async () => {
        ...
        await supabase.auth.signInWithOtp({ email: emailInput, options: { shouldCreateUser: true, data: { source: 'tools', tool_used: 'bfcm_planner' } } })
    ...
    return (
        <div className="py-12 px-6"><div className="max-w-2xl mx-auto">
            <div className="mb-10">
                ...
                <p className="text-sm text-gray-500">Black Friday ve Cyber Monday kampanyalarınızın kârlılığını önceden simüle edin.</p>
            </div>
                ...
                <div className="relative rounded-2xl border border-gray-200/80 bg-white p-6 sm:p-8 overflow-hidden">
                    ...
                    <div className={!user ? 'blur-md select-none pointer-events-none' : ''}>
                        ...
                        <div className="grid gap-3 sm:grid-cols-2 mt-6">
                            <div className="p-4 rounded-xl bg-amber-50 border border-amber-200/60 text-center"><p className="text-xs text-amber-600/70 mb-1">Kâr Marjı</p><p className={`text-2xl font-bold ${results.profitMargin > 10 ? 'text-emerald-600' : 'text-red-600'}`}>%{results.profitMargin}</p></div>
                            <div className="p-4 rounded-xl bg-amber-50 border border-amber-200/60 text-center"><p className="text-xs text-amber-600/70 mb-1">Gereken Min. ROAS</p><p className="text-2xl font-bold text-amber-700">{results.roasNeeded}x</p></div>
                        </div>
                    ...
                    {!user && (
                        <div className="absolute inset-0 flex items-center justify-center bg-white/60 backdrop-blur-sm rounded-2xl">
                            <div className="text-center max-w-sm px-6">
                                ...
                                {authMode !== 'otp-verify' && <button onClick={handleGoogleAuth} disabled={authLoading} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border border-gray-200 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors mb-3 shadow-sm"><svg className="w-4 h-4" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" /><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" /><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" /><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" /></svg>Google ile Giriş Yap</button>}
                                {authMode === 'google-waiting' && <p className="text-xs text-amber-600 mb-3 animate-pulse">Yeni sekmede giriş yapın, bu sayfa otomatik güncellenecek...</p>}
                                {authMode === 'idle' && <button onClick={() => setAuthMode('email-input')} className="w-full py-3 rounded-xl bg-amber-600 text-white text-sm font-semibold hover:bg-amber-700 transition-colors">E-posta ile Giriş Yap</button>}
                                {authMode === 'email-input' && <div className="flex gap-2"><input type="email" placeholder="ornek@email.com" value={emailInput} onChange={e => setEmailInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleEmailSubmit()} className="flex-1 px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/30" /><button onClick={handleEmailSubmit} disabled={authLoading || !emailInput} className="px-4 py-3 rounded-xl bg-amber-600 text-white text-sm font-semibold disabled:opacity-50">Gönder</button></div>}
                                {authMode === 'otp-verify' && <div><p className="text-xs text-gray-500 mb-3"><strong>{authEmail}</strong> adresine 6 haneli kod gönderildi.</p><div className="flex gap-2"><input type="text" maxLength={6} placeholder="000000" value={otpCode} onChange={e => setOtpCode(e.target.value.replace(/\D/g, ''))} onKeyDown={e => e.key === 'Enter' && handleOtpVerify()} className="flex-1 px-4 py-3 rounded-xl border border-gray-200 text-sm text-center tracking-[0.3em] font-mono focus:outline-none focus:ring-2 focus:ring-amber-500/30" /><button onClick={handleOtpVerify} disabled={authLoading || otpCode.length < 6} className="px-4 py-3 rounded-xl bg-amber-600 text-white text-sm font-semibold disabled:opacity-50">Doğrula</button></div><button onClick={() => { setAuthMode('email-input'); setOtpCode('') }} className="text-xs text-gray-400 hover:text-gray-600 mt-2">Farklı e-posta kullan</button></div>}
