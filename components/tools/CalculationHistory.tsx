...
export default function CalculationHistory({ supabase, toolName, onLoad, refreshKey, formatSummary }: HistoryPanelProps) {
    ...
    return (
        <div className="mt-8 rounded-2xl border border-gray-200/80 bg-white overflow-hidden">
            <button
            ...
            >
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-gray-700">Geçmiş Hesaplamalarınız</span>
            ...
            {open && (
                <div className="border-t border-gray-100 divide-y divide-gray-100">
                    {loading ? (
                        <div className="px-6 py-4 text-center"><p className="text-sm text-gray-400 animate-pulse">Yükleniyor...</p></div>
                    ) : history.map((calc) => (
                        <button
                            key={calc.id}
                            onClick={() => onLoad(calc.inputs, calc.results)}
                            className="w-full px-6 py-3 flex items-center justify-between text-left hover:bg-gray-50/80 transition-colors group"
                        >
                            <div className="min-w-0 flex-1">
                                <p className="text-xs text-gray-400 mb-0.5">{formatDate(calc.created_at)}</p>
                                <p className="text-sm text-gray-600 truncate group-hover:text-gray-900 transition-colors">
                                    {formatSummary(calc.results)}
                                </p>
                            </div>
                            <svg className="w-4 h-4 text-gray-300 group-hover:text-gray-500 transition-colors ml-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
