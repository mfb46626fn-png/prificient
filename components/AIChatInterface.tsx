    ...
    <>
      ...
      {!isOpen && (
        <div className="fixed bottom-28 right-8 z-[9990] animate-in fade-in slide-in-from-bottom-4 duration-300">
          <button
            ...
            className="group relative flex items-center justify-center w-16 h-16 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-2xl hover:scale-110 transition-all duration-300 border-4 border-white/20"
          >
            <BrainCircuit size={32} className="text-white animate-pulse" />

            {/* Tooltip */}
            <span className="absolute right-full mr-4 bg-blue-900 text-white text-xs font-bold px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-lg">
              Finansal Asistan
            </span>
          </button>
        </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-200">
          ...
          <div className="bg-white w-full max-w-5xl h-[85vh] rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden relative animate-in zoom-in-95 duration-300 border border-blue-50">

            {/* HEADER */}
            <div className="bg-white/95 backdrop-blur-sm p-6 flex justify-between items-center absolute top-0 w-full z-10 border-b border-blue-50">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-blue-900 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-blue-200">
            ...
            <div className="flex-1 overflow-y-auto p-8 pt-32 pb-32 space-y-8 bg-gray-50/50 scroll-smooth">
                  ...
                  <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`max-w-[80%] space-y-3`}>
                                ...
                                if (href === '../connect/shopify' || href === '/connect/shopify') {
                                  return (
                                    <a href={href} className="block mt-4 mb-2 group text-decoration-none" {...props}>
                                      <span className="bg-gradient-to-r from-blue-600 to-indigo-600 p-4 rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-[1.02] flex items-center justify-between text-white border border-white/10">
                                        <span className="flex items-center gap-3">
                                          ...
                                          <span className="flex flex-col">
                                            ...
                                            <span className="text-xs text-blue-100 opacity-90">Verileri görmek için kurulumu tamamlayın</span>
                                          </span>
                                        </span>
                                        <span className="bg-white text-blue-600 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 group-hover:bg-blue-50 transition-colors">
                      ...
                      {msg.toolInvocations?.map((toolInvocation: any) => {
                        ...
                        if (state === 'result') {
                          ...
                          if (toolName === 'query_financial_ledger') {
                            if (result.groupBy && (result.groupBy === 'day' || result.groupBy === 'month') && result.data && result.data.length > 0) {
                              return (
                                <div key={toolCallId} className="bg-white p-4 rounded-xl shadow-lg border border-blue-50">
                                  <FinancialChart
                                    data={result.data}
                        ...
                        if (state === 'call') {
                          return (
                            <div key={toolCallId} className="bg-blue-50 border border-blue-100 p-3 rounded-xl text-xs text-blue-600 font-bold animate-pulse flex items-center gap-2 w-fit">
              ...
              {isLoading && (
                <div className="flex gap-4 animate-pulse">
                  <div className="w-10 h-10 bg-blue-600 text-white rounded-full flex items-center justify-center"><Bot size={20} /></div>
                  <div className="bg-white p-5 rounded-[2rem] rounded-tl-none border border-blue-50 shadow-sm flex items-center gap-3">
            ...
            {/* INPUT ALANI */}
            <div className="absolute bottom-0 w-full bg-white p-6 border-t border-blue-50/50 backdrop-blur-xl">
              <form onSubmit={handleSendMessage} className="max-w-4xl mx-auto relative flex items-center gap-3">
                ...
                <input
                  ...
                  className="w-full bg-gray-50 pl-6 pr-16 py-5 rounded-2xl text-base font-medium text-gray-900 outline-none focus:ring-4 focus:ring-blue-50 focus:bg-white transition-all border-2 border-transparent focus:border-blue-100 placeholder:text-gray-400"
                  value={input}
